// SchichtFunk – Schicht-Marktplatz V1
(function () {
  const B = (window.SFBackend = window.SFBackend || {}),
    MANAGER = new Set(["OWNER", "ADMIN", "DISPATCHER", "PLANNER"]);
  let rows = [],
    busy = false,
    managerError = "",
    marketFilter = "active",
    marketSearch = "";
  const esc = (s) =>
    String(s ?? "").replace(
      /[&<>"']/g,
      (c) =>
        ({
          "&": "&amp;",
          "<": "&lt;",
          ">": "&gt;",
          '"': "&quot;",
          "'": "&#39;",
        })[c],
    );
  const dt = (v) =>
    new Intl.DateTimeFormat("de-DE", {
      timeZone: B.companyTimeZone || "Europe/Berlin",
      weekday: "short",
      day: "2-digit",
      month: "2-digit",
      hour: "2-digit",
      minute: "2-digit",
    }).format(new Date(v));
  const status = (s) =>
    ({
      MARKET_OPEN: "Verfügbar",
      PENDING_MANAGER: "Freigabe ausstehend",
      APPLIED: "Genehmigt",
      REJECTED_MANAGER: "Abgelehnt",
      CANCELLED: "Zurückgezogen",
      SUPERSEDED: "Nicht mehr verfügbar",
    })[s] || s;
  function css() {
    if (document.getElementById("sfMarketCss")) return;
    const s = document.createElement("style");
    s.id = "sfMarketCss";
    s.textContent = `
  #sfEmployeeSwapCard,.sf-swap-mini{display:none!important}.sf-market{border:1px solid #25435b;border-radius:13px;background:linear-gradient(180deg,#102033,#0b1827);padding:15px;margin:14px 0}.sf-market-head{display:flex;align-items:center;gap:9px;margin-bottom:11px}.sf-market-head h3{margin:0}.sf-market-head p{margin:2px 0 0;color:#8299ae;font-size:10px}.sf-market-count,.sf-market-state{border:1px solid #2b675a;border-radius:999px;background:#0e2b25;color:#7ce5cf;padding:4px 8px;font-size:9px;font-weight:900}.sf-market-count{margin-left:auto}.sf-market-list{display:grid;gap:8px}.sf-market-row{display:grid;grid-template-columns:minmax(210px,1fr) auto auto;align-items:center;gap:14px;border:1px solid #233e54;border-radius:10px;background:#091725;padding:11px}.sf-market-row>div:nth-child(2){display:flex;flex-direction:column;align-items:flex-end;justify-self:end}.sf-market-row b,.sf-market-row small{display:block}.sf-market-row small{color:#8198ac;font-size:9px;line-height:1.5;margin-top:3px}.sf-market-actions{display:flex;gap:6px;justify-self:end}.sf-market-actions:empty{display:none}.sf-market-actions button,.sf-market-offer{border-radius:8px;padding:7px 9px;font-size:9px;font-weight:900}.sf-market-take,.sf-market-approve,.sf-market-offer{border:1px solid #28d7b5;background:#28d7b5;color:#05251f}.sf-market-reject{border:1px solid #693642;background:#2d1921;color:#ffa0ad}.sf-market-cancel{border:1px solid #38536a;background:#0b1926;color:#b2c3d1}.sf-market-empty{padding:18px;text-align:center;border:1px dashed #2a445a;border-radius:9px;color:#8299ac}.sf-market-block{color:#d7a95d!important}.sf-market-modal{position:fixed;inset:0;z-index:38000;background:rgba(2,7,13,.9);display:grid;place-items:center;padding:18px}.sf-market-dialog{width:min(540px,96vw);background:#0d1b2a;border:1px solid #2b4961;border-radius:16px;overflow:hidden}.sf-market-dialog header,.sf-market-dialog main,.sf-market-dialog footer{padding:16px 19px}.sf-market-dialog header{border-bottom:1px solid #22394d}.sf-market-dialog h2{margin:4px 0}.sf-market-dialog p{color:#8ea4b8;font-size:11px}.sf-market-dialog textarea{width:100%;min-height:85px;box-sizing:border-box;background:#081522;color:#eef6ff;border:1px solid #29445b;border-radius:9px;padding:10px}.sf-market-dialog footer{display:flex;justify-content:flex-end;gap:8px;border-top:1px solid #22394d}.sf-market-dialog footer button{padding:9px 12px;border-radius:8px}.sf-market-kpis{grid-template-columns:repeat(4,minmax(0,1fr))}.sf-market-tabs{display:flex;gap:7px;flex-wrap:wrap;margin:14px 0 10px}.sf-market-tabs button{border:1px solid #29465d;background:#0a1826;color:#9eb2c5;border-radius:8px;padding:8px 11px}.sf-market-tabs button.active{border-color:#28d7b5;background:#10342d;color:#8aecd7}.sf-market-toolbar{display:flex;gap:10px;align-items:center}.sf-market-toolbar input{flex:1;min-width:180px;background:#081522;border:1px solid #29445b;color:#eef6ff;border-radius:9px;padding:10px 11px}.sf-market-open-dashboard{margin-left:auto}.sf-market-view .sf-market{margin-top:0}.sf-market-section-title{display:flex;align-items:center;justify-content:space-between;margin:14px 0 7px}.sf-market-section-title b{font-size:11px}.sf-market-note{border:1px solid #275146;background:#0d2924;color:#8bdfce;border-radius:8px;padding:9px 10px;font-size:9px;line-height:1.5;margin-bottom:11px}@media(max-width:900px){.sf-market-kpis{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:720px){.sf-market-row{grid-template-columns:1fr}.sf-market-row>div:nth-child(2){align-items:flex-start;justify-self:start}.sf-market-actions{justify-content:flex-start;justify-self:start}.sf-market-head{align-items:flex-start;flex-wrap:wrap}.sf-market-count{margin-left:0}.sf-market-toolbar{align-items:stretch;flex-direction:column}.sf-market-kpis{grid-template-columns:1fr}}
  `;
    document.head.appendChild(s);
  }
  function modal(title, text, action, run) {
    css();
    document.getElementById("sfMarketModal")?.remove();
    const m = document.createElement("div");
    m.id = "sfMarketModal";
    m.className = "sf-market-modal";
    m.innerHTML = `<section class="sf-market-dialog" role="dialog" aria-modal="true" aria-labelledby="sfMarketTitle" tabindex="-1"><header><div class="eyebrow">SCHICHT-MARKTPLATZ</div><h2 id="sfMarketTitle">${esc(title)}</h2><p id="sfMarketDescription">${esc(text)}</p></header><main><label for="sfMarketNote">Hinweis (optional)</label><textarea id="sfMarketNote" maxlength="1000" placeholder="Kurze Information …" aria-describedby="sfMarketDescription"></textarea><p class="sf-market-error" role="alert" aria-live="assertive"></p></main><footer><button class="ghost" data-close>Abbrechen</button><button class="sf-market-approve" data-submit>${esc(action)}</button></footer></section>`;
    document.body.appendChild(m);
    const close = B.bindAccessibleModal?.(m, { initialFocus: "textarea" }) || (() => m.remove());
    m.querySelector("[data-close]").onclick = close;
    m.onclick = (e) => {
      if (e.target === m) close();
    };
    m.querySelector("[data-submit]").onclick = async (e) => {
      const b = e.currentTarget;
      b.disabled = true;
      b.textContent = "Wird gespeichert …";
      try {
        await run(m.querySelector("textarea").value.trim());
        close();
      } catch (x) {
        if (x?.code === "OFFER_UNAVAILABLE") {
          close();
          showSaveToast?.(
            "Angebot bereits vergeben",
            "Der Marktplatz wurde aktualisiert.",
          );
          return;
        }
        m.querySelector(".sf-market-error").textContent = x.message || x;
        b.disabled = false;
        b.textContent = action;
      }
    };
  }
  async function rpc(name, args = {}) {
    if (!B.client) throw Error("Cloud-Verbindung ist noch nicht bereit.");
    const q = await B.client.rpc(name, args);
    if (q.error) throw q.error;
    return q.data;
  }
  async function employeeLoad() {
    if (busy || B.role !== "EMPLOYEE") return;
    busy = true;
    try {
      rows = (await rpc("employee_list_shift_marketplace")) || [];
      renderEmployee();
    } catch (e) {
      console.warn("Marktplatz", e);
    } finally {
      busy = false;
    }
  }
  function upcoming() {
    return (B.employeePortalData?.shifts || []).filter(
      (x) => x.published_at && new Date(x.starts_at) > new Date(),
    );
  }
  function addOfferButtons() {
    const root = document.getElementById("sfEmployeePortal");
    if (!root) return;
    const shifts = upcoming(),
      items = root.querySelectorAll(".sf-shift-item");
    items.forEach((el, i) => {
      const sh = shifts[i];
      if (!sh) return;
      const active = rows.some(
        (x) =>
          x.assignment_id === sh.id &&
          x.is_own &&
          ["MARKET_OPEN", "PENDING_COLLEAGUE", "PENDING_MANAGER"].includes(
            x.status,
          ),
      );
      let b = el.querySelector(".sf-market-offer");
      if (!b) {
        b = document.createElement("button");
        b.className = "sf-market-offer";
        b.type = "button";
        el.appendChild(b);
      }
      b.disabled = active;
      b.textContent = active ? "✓ Im Marktplatz" : "↺ Im Marktplatz anbieten";
      b.onclick = active
        ? null
        : () =>
            modal(
              "Schicht anbieten",
              `${sh.shift_code} · ${dt(sh.starts_at)}`,
              "Angebot veröffentlichen",
              async (note) => {
                try {
                  await rpc("employee_offer_shift_marketplace", {
                    p_assignment_id: sh.id,
                    p_reason: note,
                  });
                } catch (e) {
                  if (!String(e?.message || e).includes("bereits ein Angebot"))
                    throw e;
                }
                await employeeLoad();
                B.notifications?.refresh?.();
                if (typeof showSaveToast === "function")
                  showSaveToast(
                    "Angebot veröffentlicht",
                    "Die Schicht ist jetzt im Marktplatz sichtbar.",
                  );
              },
            );
    });
  }
  function renderEmployee() {
    const root = document.getElementById("sfEmployeePortal");
    if (!root) return;
    css();
    root.querySelector("#sfMarketEmployee")?.remove();
    const host = root.querySelector(".sf-portal-grid") || root;
    const card = document.createElement("section");
    card.id = "sfMarketEmployee";
    card.className = "sf-portal-card sf-market";
    card.dataset.sfPortalSection = "marketplace";
    const available = rows.filter(
        (x) => x.status === "MARKET_OPEN" && !x.is_own,
      ),
      mine = rows.filter(
        (x) => x.is_own || (!x.is_own && x.status !== "MARKET_OPEN"),
      );
    const line = (x) =>
      `<article class="sf-market-row" data-id="${x.id}"><div><b>${esc(x.shift_code)} · ${esc(dt(x.starts_at))}</b><small>${x.is_own ? "Eigenes Angebot" : "Angeboten von " + esc(x.offered_by)}${x.reason ? " · " + esc(x.reason) : ""}</small></div><div><span class="sf-market-state">${esc(status(x.status))}</span>${x.block_reason ? `<small class="sf-market-block">${esc(x.block_reason)}</small>` : ""}</div><div class="sf-market-actions">${x.status === "MARKET_OPEN" && !x.is_own && x.can_take ? '<button class="sf-market-take" data-take>Schicht übernehmen</button>' : ""}${x.is_own && ["MARKET_OPEN", "PENDING_MANAGER"].includes(x.status) ? '<button class="sf-market-cancel" data-cancel>Zurückziehen</button>' : ""}</div></article>`;
    card.innerHTML = `<div class="sf-market-head"><div><h3>Schicht-Marktplatz</h3><p>Eigene Schichten anbieten und passende Angebote übernehmen</p></div><span class="sf-market-count">${available.filter((x) => x.can_take).length} passend</span></div><div class="sf-market-note">Eine Übernahme ändert den Dienstplan noch nicht. Sie wird zuerst der Disposition beziehungsweise einer berechtigten Fachkraft zur Prüfung vorgelegt.</div><div class="sf-market-section-title"><b>Verfügbare Schichten</b><span class="sf-market-count">${available.length}</span></div><div class="sf-market-list">${available.length ? available.map(line).join("") : '<div class="sf-market-empty">Aktuell werden keine Schichten angeboten.</div>'}</div><div class="sf-market-section-title"><b>Meine Angebote und Übernahmen</b><span class="sf-market-count">${mine.length}</span></div><div class="sf-market-list">${mine.length ? mine.map(line).join("") : '<div class="sf-market-empty">Noch keine eigenen Marktplatzvorgänge.</div>'}</div>`;
    host.insertBefore(card, host.firstChild);
    card.querySelectorAll("[data-take]").forEach(
      (b) =>
        (b.onclick = () => {
          const x = rows.find(
            (r) => r.id === b.closest("[data-id]").dataset.id,
          );
          modal(
            "Schicht übernehmen",
            `${x.shift_code} · ${dt(x.starts_at)} · von ${x.offered_by}`,
            "Zur Prüfung einreichen",
            async (note) => {
              try {
                await rpc("employee_claim_shift_marketplace", {
                  p_offer_id: x.id,
                  p_comment: note,
                });
              } catch (error) {
                if (!/nicht mehr verfügbar|bereits vergeben/i.test(String(error?.message || error)))
                  throw error;
                await employeeLoad();
                B.notifications?.refresh?.();
                const unavailable = new Error(
                  "Dieses Angebot wurde bereits vergeben.",
                );
                unavailable.code = "OFFER_UNAVAILABLE";
                throw unavailable;
              }
              await employeeLoad();
              B.notifications?.refresh?.();
              showSaveToast?.(
                "Übernahme eingereicht",
                "Disposition oder Fachprüfung wurde informiert.",
              );
            },
          );
        }),
    );
    card.querySelectorAll("[data-cancel]").forEach(
      (b) =>
        (b.onclick = async () => {
          await rpc("employee_cancel_shift_swap", {
            p_swap_id: b.closest("[data-id]").dataset.id,
          });
          await employeeLoad();
        }),
    );
    addOfferButtons();
  }
  async function managerLoad() {
    if (!MANAGER.has(B.role) || !B.companyId || busy) return;
    busy = true;
    managerError = "";
    try {
      rows =
        (await rpc("manager_list_shift_marketplace", {
          p_company_id: B.companyId,
        })) || [];
      renderManager();
    } catch (e) {
      managerError = e?.message || "Daten konnten nicht geladen werden.";
      console.warn("Marktplatz", e);
      renderDashboard();
    } finally {
      busy = false;
    }
  }
  function reviewButton(x) {
    return x.status === "PENDING_MANAGER"
      ? '<button class="sf-market-reject" data-review="REJECT">Ablehnen</button><button class="sf-market-approve" data-review="APPROVE">Freigeben</button>'
      : "";
  }
  function bindReviews(host) {
    host.querySelectorAll("[data-review]").forEach((b) => {
      const x = rows.find((r) => r.id === b.closest("[data-id]").dataset.id);
      b.onclick = () =>
        modal(
          b.dataset.review === "APPROVE"
            ? "Übernahme freigeben"
            : "Übernahme ablehnen",
          `${x.offered_by} → ${x.claimed_by} · ${x.shift_code}`,
          b.dataset.review === "APPROVE" ? "Freigeben" : "Ablehnen",
          async (note) => {
            await rpc("manager_review_shift_swap", {
              p_swap_id: x.id,
              p_decision: b.dataset.review,
              p_comment: note,
            });
            await B.hydrate?.();
            renderCalendar?.();
            await managerLoad();
            showSaveToast?.(
              "Entscheidung gespeichert",
              "Der Dienstplan wurde entsprechend aktualisiert.",
            );
          },
        );
    });
  }
  function openDashboard() {
    ensureDashboard();
    B.persistWorkspaceView?.("marketplace");
    B.pendingView = "marketplace";
    window.switchView?.("marketplace");
    renderDashboard();
    managerLoad();
    window.scrollTo({ top: 0, behavior: "instant" });
  }
  function ensureDashboard() {
    if (!MANAGER.has(B.role)) return;
    css();
    const nav = document.getElementById("nav");
    if (nav && !nav.querySelector('[data-view="marketplace"]')) {
      const b = document.createElement("button");
      b.type = "button";
      b.dataset.view = "marketplace";
      b.innerHTML = "<span>↺</span><span>Schicht-Marktplatz</span>";
      const before = nav.querySelector('[data-nav-group="personal"]') || nav.querySelector('[data-view="reports"]');
      before ? nav.insertBefore(b, before) : nav.appendChild(b);
    }
    const marketButton = nav?.querySelector('[data-view="marketplace"]');
    if (marketButton) marketButton.onclick = openDashboard;
    const content = document.querySelector("#appShell .content");
    if (content && !document.getElementById("view-marketplace")) {
      const v = document.createElement("section");
      v.id = "view-marketplace";
      v.className = "view sf-market-view";
      v.innerHTML =
        '<div class="page-head"><div><div class="eyebrow">ZUSAMMENARBEIT</div><h1>Schicht-Marktplatz</h1><p>Angebote, Übernahmewünsche und Planerfreigaben zentral verwalten.</p></div><button class="ghost" id="sfMarketRefresh">↻ Aktualisieren</button></div><div class="stats sf-market-kpis" id="sfMarketKpis"></div><div class="sf-market-tabs" id="sfMarketTabs"><button data-filter="active">Aktuell</button><button data-filter="available">Verfügbar</button><button data-filter="review">Zu prüfen</button><button data-filter="history">Verlauf</button></div><section class="sf-market"><div class="sf-market-toolbar"><input id="sfMarketSearch" type="search" placeholder="Mitarbeiter, Schicht oder Status suchen …"><span class="sf-market-count" id="sfMarketResultCount">0 Einträge</span></div><div class="sf-market-list" id="sfMarketDashboardList" style="margin-top:11px"></div></section>';
      content.appendChild(v);
      v.querySelector("#sfMarketRefresh").onclick = managerLoad;
      v.querySelector("#sfMarketSearch").oninput = (e) => {
        marketSearch = e.target.value.trim().toLowerCase();
        renderDashboard();
      };
      v.querySelectorAll("[data-filter]").forEach(
        (b) =>
          (b.onclick = () => {
            marketFilter = b.dataset.filter;
            renderDashboard();
          }),
      );
    }
  }
  function filteredRows() {
    return rows.filter((x) => {
      const group =
        marketFilter === "active"
          ? ["MARKET_OPEN", "PENDING_MANAGER"].includes(x.status)
          : marketFilter === "available"
            ? x.status === "MARKET_OPEN"
            : marketFilter === "review"
              ? x.status === "PENDING_MANAGER"
              : !["MARKET_OPEN", "PENDING_MANAGER"].includes(x.status);
      const text =
        `${x.shift_code} ${x.offered_by} ${x.claimed_by || ""} ${status(x.status)} ${x.reason || ""}`.toLowerCase();
      return group && (!marketSearch || text.includes(marketSearch));
    });
  }
  function renderDashboard() {
    ensureDashboard();
    const view = document.getElementById("view-marketplace");
    if (!view) return;
    const open = rows.filter((x) => x.status === "MARKET_OPEN").length,
      pending = rows.filter((x) => x.status === "PENDING_MANAGER").length,
      done = rows.filter((x) => x.status === "APPLIED").length,
      closed = rows.filter((x) =>
        ["REJECTED_MANAGER", "CANCELLED", "SUPERSEDED"].includes(x.status),
      ).length;
    view.querySelector("#sfMarketKpis").innerHTML =
      `<div class="stat"><div class="stat-icon">↺</div><div><small>Verfügbare Schichten</small><strong>${open}</strong><em>im Marktplatz</em></div></div><div class="stat"><div class="stat-icon">△</div><div><small>Planerfreigaben</small><strong>${pending}</strong><em>zu prüfen</em></div></div><div class="stat"><div class="stat-icon">✓</div><div><small>Genehmigt</small><strong>${done}</strong><em>im Verlauf</em></div></div><div class="stat"><div class="stat-icon">◇</div><div><small>Abgeschlossen</small><strong>${closed}</strong><em>abgelehnt oder beendet</em></div></div>`;
    view
      .querySelectorAll("[data-filter]")
      .forEach((b) =>
        b.classList.toggle("active", b.dataset.filter === marketFilter),
      );
    const data = filteredRows();
    view.querySelector("#sfMarketResultCount").textContent = managerError
      ? "Fehler"
      : `${data.length} ${data.length === 1 ? "Eintrag" : "Einträge"}`;
    view.querySelector("#sfMarketDashboardList").innerHTML = managerError
      ? `<div class="sf-market-empty sf-market-block">Marktplatz konnte nicht geladen werden: ${esc(managerError)}<br>Bitte erneut aktualisieren.</div>`
      : data.length
        ? data
            .map(
              (x) =>
                `<article class="sf-market-row" data-id="${x.id}"><div><b>${esc(x.shift_code)} · ${esc(dt(x.starts_at))}</b><small>${esc(x.offered_by)}${x.claimed_by ? " → " + esc(x.claimed_by) : " · noch nicht übernommen"}${x.reason ? " · " + esc(x.reason) : ""}</small></div><div><span class="sf-market-state">${esc(status(x.status))}</span>${x.colleague_comment ? `<small>${esc(x.colleague_comment)}</small>` : ""}</div><div class="sf-market-actions">${reviewButton(x)}</div></article>`,
            )
            .join("")
        : '<div class="sf-market-empty">Für diesen Filter sind keine Einträge vorhanden.</div>';
    bindReviews(view);
  }
  function renderManager() {
    if (!MANAGER.has(B.role)) return;
    ensureDashboard();
    renderDashboard();
    const view = document.getElementById("view-overview");
    if (!view) return;
    css();
    view.querySelector("#sfMarketManager")?.remove();
    const active = rows.filter((x) =>
      ["MARKET_OPEN", "PENDING_MANAGER"].includes(x.status),
    );
    const box = document.createElement("section");
    box.id = "sfMarketManager";
    box.className = "sf-market";
    box.innerHTML = `<div class="sf-market-head"><div><h3>Schicht-Marktplatz</h3><p>Offene Angebote und Übernahmen zur Planerfreigabe</p></div><button class="ghost sf-market-open-dashboard" id="sfOpenMarketDashboard">Dashboard öffnen →</button><span class="sf-market-count">${active.filter((x) => x.status === "PENDING_MANAGER").length} zu prüfen</span></div><div class="sf-market-list">${
      active.length
        ? active
            .slice(0, 4)
            .map(
              (x) =>
                `<article class="sf-market-row" data-id="${x.id}"><div><b>${esc(x.shift_code)} · ${esc(dt(x.starts_at))}</b><small>${esc(x.offered_by)}${x.claimed_by ? " → " + esc(x.claimed_by) : " · noch nicht übernommen"}</small></div><span class="sf-market-state">${esc(status(x.status))}</span><div class="sf-market-actions">${reviewButton(x)}</div></article>`,
            )
            .join("")
        : '<div class="sf-market-empty">Keine offenen Angebote oder Freigaben.</div>'
    }</div>`;
    const anchor = view.querySelector("#sfDashboardRoot");
    anchor ? anchor.appendChild(box) : view.appendChild(box);
    box.querySelector("#sfOpenMarketDashboard").onclick = openDashboard;
    bindReviews(box);
  }
  function refresh() {
    if (B.role === "EMPLOYEE" && document.getElementById("sfEmployeePortal"))
      employeeLoad();
    else if (
      MANAGER.has(B.role) &&
      document.querySelector("#view-overview.active,#view-marketplace.active")
    )
      managerLoad();
  }
  const timer = setInterval(() => {
    if (B.client && B.role) {
      clearInterval(timer);
      ensureDashboard();
      const base = B.openEmployeePortal;
      if (typeof base === "function")
        B.openEmployeePortal = function () {
          const r = base.apply(this, arguments);
          setTimeout(employeeLoad, 50);
          return r;
        };
      const old = window.renderOverview;
      if (typeof old === "function")
        window.renderOverview = function () {
          const r = old.apply(this, arguments);
          setTimeout(managerLoad, 50);
          return r;
        };
      refresh();
      setInterval(() => {
        if (MANAGER.has(B.role)) refresh();
      }, 15000);
    }
  }, 250);
  document.addEventListener(
    "click",
    (e) => {
      if (e.target.closest('[data-view="marketplace"]')) {
        openDashboard();
        return;
      }
      if (e.target.closest('[data-view="overview"]'))
        setTimeout(managerLoad, 100);
    },
    true,
  );
})();
