...
(function(){
  ...
  function detail(r){
    ...
    m.innerHTML=`<section class="sf-audit-detail" role="dialog" aria-modal="true" aria-labelledby="sfAuditDetailTitle"><header><div><div class="eyebrow">AUDIT-DETAIL</div><h2 id="sfAuditDetailTitle">${esc(actionLabel(r))}</h2><small>${esc(r.entity_type||'System')} · ${esc(r.entity_id||r.id)}</small></div><button type="button" class="ghost sf-audit-close" aria-label="Schließen">✕</button></header><div class="sf-audit-detail-body"><div class="sf-audit-meta"><div><small>Zeitstempel</small><b>${esc(dateTime(r.created_at))}</b></div><div><small>Benutzer</small><b>${esc(r.actor_email||'System')}</b></div><div><small>Rolle</small><b>${esc(roleLabels[r.actor_role]||r.actor_role||'System')}</b></div></div><div class="sf-audit-diff"><div class="sf-audit-change"><b>Vorher</b><pre>${json(r.old_values)}</pre></div><div class="sf-audit-change"><b>Nachher</b><pre>${json(r.new_values)}</pre></div></div></div></section>`;
