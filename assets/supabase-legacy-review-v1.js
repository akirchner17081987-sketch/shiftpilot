...
(function(){
  ...
  B.loadLegacyReview=async()=>{
    ...
    B.legacyReviewItems=(q.data||[]).map(row=>{
      const m=row.metadata||{},a=(window.assignments||[]).find(x=>String(x.id)===String(m.legacy_id));
      const e=a?(window.employees||[]).find(x=>String(x.id)===String(a.employeeId)):null;
  ...
  B.renderLegacyReview=()=>{
    ...
    card.innerHTML=`<div class="table-head" style="align-items:center"><h3>Altbestand prüfen</h3><span class="badge" style="${items.length?'background:#8a5a18':'background:#17654f'}">${items.length}</span><span style="margin-left:auto;color:var(--muted);font-size:11px">Aus PostgreSQL · Audit-Log</span></div>${items.length?`<div style="display:flex;flex-direction:column;gap:8px">${items.map(x=>{const emp=x.employee?`${x.employee.first||''} ${x.employee.last||''}`.trim():'Mitarbeiter nicht zugeordnet';return `<div class="section-box" style="display:grid;grid-template-columns:minmax(180px,1.25fr) minmax(135px,.8fr) minmax(250px,1.6fr) auto;gap:12px;align-items:center;border-color:#6d4a22;background:#241c13"><div><b>${esc(emp)}</b><small style="display:block;color:#caa87a;margin-top:3px">${esc(x.shiftCode)} · ${esc(fmtDate(x.startsAt))}</small></div><div><b>${esc(fmtTime(x.startsAt))} – ${esc(fmtTime(x.endsAt))}</b><small style="display:block;color:#a99172;margin-top:3px">übernommener Altbestand</small></div><div><span style="color:#ffc36f;font-weight:800">⚠ ${esc(reason(x.warning))}</span><small style="display:block;color:#a99172;margin-top:3px">Neue Schichten werden weiterhin durch PostgreSQL blockiert, wenn diese Regel verletzt wird.</small></div><button class="ghost" onclick="SFBackend.openLegacyAssignment('${esc(x.legacyId)}')">Im Dienstplan anzeigen</button></div>`}).join('')}</div>`:'<div class="notice">✓ Keine prüfpflichtigen Alt-Schichten vorhanden.</div>'}`;
  };
