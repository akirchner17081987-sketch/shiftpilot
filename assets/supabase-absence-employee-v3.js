...
(function(){
  ...
  function open(){
    ...
    m.innerHTML=`<div class="sf-ae3-card" role="dialog" aria-modal="true"><div class="sf-ae3-mh"><div><div class="eyebrow">ABWESENHEITSANTRAG</div><h2>Abwesenheit melden</h2><p>Der Antrag wird an die Dienstplanung übermittelt.</p></div><button type="button" class="sf-ae3-x">✕</button></div><div class="sf-ae3-body"><div class="sf-ae3-field"><label>Art</label><select id="sfAe3Type">${TYPES.map(x=>`<option>${esc(x)}</option>`).join('')}</select></div><div class="sf-ae3-grid"><div class="sf-ae3-field"><label>Von</label><input id="sfAe3From" type="date" value="${today}"></div><div class="sf-ae3-field"><label>Bis</label><input id="sfAe3To" type="date" value="${today}"></div></div><div class="sf-ae3-field"><label>Umfang</label><select id="sfAe3Full"><option value="1">Ganztägig</option><option value="0">Teil des Tages</option></select></div><div id="sfAe3Times" class="sf-ae3-grid" style="display:none"><div class="sf-ae3-field"><label>Beginn</label><input id="sfAe3Start" type="time"></div><div class="sf-ae3-field"><label>Ende</label><input id="sfAe3End" type="time"></div></div><div class="sf-ae3-field"><label>Bemerkung (optional)</label><textarea id="sfAe3Note" rows="3" maxlength="2000"></textarea></div><div class="sf-ae3-help">Der Antrag ist zunächst „In Prüfung“. Erst nach Freigabe wird die Abwesenheit planungswirksam.</div><div class="sf-ae3-msg" id="sfAe3Msg"></div></div><div class="sf-ae3-foot"><button type="button" class="ghost" id="sfAe3Cancel">Abbrechen</button><button type="button" class="primary" id="sfAe3Submit">Antrag senden</button></div></div>`;
    document.body.appendChild(m);
    const close=()=>m.remove(),msg=m.querySelector('#sfAe3Msg');
    ...
    m.querySelector('#sfAe3Full').onchange=e=>m.querySelector('#sfAe3Times').style.display=e.target.value==='0'?'grid':'none';
