...
(function(){
  ...
  async function render(container){
    ...
    try{
      ...
      const approvedIsCurrent=!approved||rulesMatchDefault(approved.rules);
      const draftIsCurrent=!draft||rulesMatchDefault(draft.rules);
      const shown=draft&&!draftIsCurrent||approved&&!approvedIsCurrent?DEFAULT_RULES:(draft?.rules||approved?.rules||DEFAULT_RULES);
      ...
      const state=approved&&!approvedIsCurrent&&draft&&draftIsCurrent
        ?`<div class="sf-pr-state bad"><b>Aktives Fristprofil V${approved.version} ist fachlich überholt.</b><br>Bis zur sicheren Ablösung bleiben dessen Regeln aktiv.</div><div class="sf-pr-state wait"><b>Korrektes Fristprofil V${draft.version} wartet auf zweite Bestätigung.</b><br>Frühestens: ${esc(deDate(draft.confirmation_not_before))}. Nach Bestätigung wird V${approved.version} atomar widerrufen.</div>`
        :approved&&!approvedIsCurrent
          ?`<div class="sf-pr-state bad"><b>Aktives Fristprofil V${approved.version} ist fachlich überholt.</b><br>Zeitnachweise stehen noch auf ${esc(approved.rules?.timeEvidenceYears)} Jahre statt 6 Jahren. Bereiten Sie V${nextVersion} mit MFA vor; V${approved.version} bleibt bis zur zweiten geschützten Bestätigung aktiv.</div>`
        :approved
      ...
      const canStage=!draft&&(!approved||!approvedIsCurrent);
      const needsProfileInput=canStage||(!approved&&draft&&!draftIsCurrent);
      host.innerHTML=`<h3>Datenschutz & Löschung</h3><p>Technische Schutzregeln sind aktiv. Konkrete Fristen werden erst nach geschützter Betreiberfreigabe wirksam. Vor der Freigabe müssen die Werte vertraglich/rechtlich zum tatsächlichen Einsatz passen.</p>${state}<div class="sf-pr-grid">${rulesSummary(shown).map(([k,v])=>`<div class="sf-pr-row"><span>${esc(k)}</span><b>${esc(v)}</b></div>`).join('')}</div>${needsProfileInput?`<label style="display:block;margin-top:12px;font-size:11px;color:#9fb3c7">Freigabevermerk<input id="sfPrApprovalRef" class="sf-pr-ref" maxlength="500" value="SchichtFunk Fristprofil V${nextVersion} – Arbeitszeit/DATEV 6 Jahre; ersetzt V${approved?.version||draft?.version||'—'}"></label>`:''}<div id="sfPrMessage"></div><div class="sf-pr-actions"><button class="ghost" id="sfPrReload" type="button">Status neu laden</button>${canStage?'<button class="primary" id="sfPrStage" type="button">Korrektes V2 vorbereiten</button>':''}${!approved&&draft&&!draftIsCurrent?'<button class="primary" id="sfPrReplace" type="button">Entwurf durch V2 ersetzen</button>':''}${draft&&draftIsCurrent?`<button class="primary" id="sfPrConfirm" type="button" ${canConfirm?'':'disabled'}>${canConfirm?'V2 aktivieren und V1 ablösen':'Abkühlfrist läuft'}</button>`:''}</div>`;
