...
(function(){
  ...
  let busy=false,lastSync='',rendering=false,renderAgain=false;
  ...
  document.addEventListener('click',e=>{if(e.target.closest?.('[data-sf-push-enable]')){e.preventDefault();enable()}else if(e.target.closest?.('[data-sf-push-disable]')){e.preventDefault();disable()}else if(e.target.closest?.('[data-sf-push-test]')){e.preventDefault();sendTest()}},true);
