...
(function(){
  ...
  B.openEmployeePortal=function(){
    css();const d=B.employeePortalData;if(!d)return;document.getElementById('landingPage')?.style.setProperty('display','none');document.getElementById('appShell')?.style.setProperty('display','none');document.getElementById('sfEmployeePortal')?.remove();const now=Date.now(),upcoming=d.shifts.filter(s=>new Date(s.ends_at).getTime()>=now).sort((a,b)=>new Date(a.starts_at)-new Date(b.starts_at)),next=upcoming[0],weekEnd=now+7*86400000,weekHours=upcoming.filter(s=>new Date(s.starts_at).getTime()<weekEnd).reduce((n,s)=>n+Math.max(0,(new Date(s.ends_at)-new Date(s.starts_at))/3600000-(Number(s.break_minutes||0)/60)),0),name=`${d.employee.first_name||''} ${d.employee.last_name||''}`.trim();
    const shiftHtml=upcoming.length?upcoming.slice(0,20).map(s=>`<div class="sf-shift-item"><div class="sf-shift-code">${esc(s.shift_code)}</div><div class="sf-item-main"><b>${esc(fmtDate(s.starts_at))}</b><small>${esc(fmtTime(s.starts_at))} – ${esc(fmtTime(s.ends_at))} · Pause ${Number(s.break_minutes||0)} Min.</small></div><span class="sf-item-state">${s.published_at?'Veröffentlicht':esc(s.status)}</span></div>`).join(''):'<div class="sf-empty">Aktuell sind keine veröffentlichten kommenden Schichten vorhanden.</div>';
    ...
    const p=document.createElement('div');p.id='sfEmployeePortal';p.className='sf-portal';p.innerHTML=`<div class="sf-portal-top"><div class="sf-portal-logo">SF</div><div class="sf-portal-brand"><b>SchichtFunk</b><small>${esc(d.company.name||'Mitarbeiterportal')}</small></div><div class="spacer"></div><button class="ghost" id="sfEmployeeLogout">Abmelden</button></div><main class="sf-portal-main"><div class="sf-portal-welcome"><div class="eyebrow">MITARBEITERPORTAL</div><h1>Hallo ${esc(d.employee.first_name||name)}</h1><p>Hier siehst du ausschließlich deine persönlichen SchichtFunk-Daten.</p></div><div class="sf-portal-stats"><div class="sf-portal-stat"><small>Kommende Schichten</small><strong>${upcoming.length}</strong></div><div class="sf-portal-stat"><small>Nächste Schicht</small><strong>${next?esc(next.shift_code)+' · '+esc(fmtDate(next.starts_at)):'–'}</strong></div><div class="sf-portal-stat"><small>Geplante Stunden · 7 Tage</small><strong>${weekHours.toFixed(1)} h</strong></div></div><div class="sf-portal-grid"><section class="sf-portal-card"><h3>Meine Schichten</h3>${shiftHtml}</section><div style="display:grid;gap:14px"><section class="sf-portal-card"><h3>Schichtänderungen</h3>${reqHtml}</section><section class="sf-portal-card"><h3>Abwesenheiten</h3>${absHtml}</section><section class="sf-portal-card"><h3>Mein Profil</h3><div class="sf-profile-list"><div class="sf-profile-field"><small>Personalnummer</small><b>${esc(d.employee.personnel_no||'–')}</b></div><div class="sf-profile-field"><small>Beschäftigung</small><b>${esc(d.employee.employment||'–')}</b></div><div class="sf-profile-field"><small>Wochenstunden</small><b>${Number(d.employee.weekly_hours||0)} h</b></div><div class="sf-profile-field"><small>E-Mail</small><b>${esc(d.employee.email||B.user?.email||'–')}</b></div><div class="sf-profile-field"><small>Telefon</small><b>${esc(d.employee.phone||'–')}</b></div><div class="sf-profile-field"><small>Arbeitszeitmodell</small><b>${esc(d.employee.work_time_model||'SHIFT')}</b></div></div></section></div></div></main>`;document.body.appendChild(p);p.querySelector('#sfEmployeeLogout').onclick=()=>B.client.auth.signOut();
  };

  const staffBoot=B.boot;if(typeof staffBoot==='function')B.boot=async function(session){if(!session?.user)return;B.user=session.user;await B.ensureCompany();if(B.role!=='EMPLOYEE')return staffBoot.apply(this,arguments);if(B.employeeBootPromise)return B.employeeBootPromise;B.employeeBootPromise=(async()=>{B.showLoading?.('Mitarbeiterportal wird geladen …');await B.hydrateEmployee();B.ready=true;B.updateState?.();B.hideLoading?.();B.openEmployeePortal();return true})().finally(()=>B.employeeBootPromise=null);return B.employeeBootPromise};
