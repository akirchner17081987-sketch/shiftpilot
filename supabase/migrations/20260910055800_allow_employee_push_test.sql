...
begin
  ...
  if v_role is null then
    ...
    limit 1;
    if v_employee_id is null then raise exception 'ACTIVE_MEMBERSHIP_REQUIRED'; end if;
    v_role := 'EMPLOYEE';
  end if;

  if not exists(
    select 1 from public.push_subscriptions ps
    where ps.user_id=v_user and ps.enabled=true
  ) then raise exception 'PUSH_NOT_ACTIVE'; end if;
  ...
  insert into public.notifications(
    company_id,user_id,employee_id,kind,title,message,link_view,entity_type,entity_id,metadata
  )
  values(
    p_company_id,v_user,v_employee_id,'SYSTEM_ENABLED','SchichtFunk Push-Test',
    'Push-Mitteilungen funktionieren auf diesem Gerät.',v_link,'PUSH_TEST',gen_random_uuid(),
    jsonb_build_object('push_test',true)
  )
