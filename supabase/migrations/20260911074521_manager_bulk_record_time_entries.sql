...
begin
  ...
  select count(*) into v_skipped_unpublished
  from public.shift_assignments sa
  where sa.company_id=p_company_id and sa.status not in ('PUBLISHED','CANCELLED')
    and sa.starts_at::date between p_start_date and p_end_date;
  ...
  where sa.company_id=p_company_id and sa.status='PUBLISHED' and te.assignment_id is null
    and sa.starts_at::date between p_start_date and p_end_date and sa.ends_at>now();

  select count(*) into v_skipped_closed
  from public.shift_assignments sa
  left join public.time_entries te on te.assignment_id=sa.id and te.company_id=sa.company_id
  where sa.company_id=p_company_id and sa.status='PUBLISHED' and te.assignment_id is null
    and sa.starts_at::date between p_start_date and p_end_date and sa.ends_at<=now()
    and private.sf_is_time_month_closed(sa.company_id,sa.starts_at::date);
  ...
  where sa.company_id=p_company_id and sa.status='PUBLISHED' and te.assignment_id is null
    and sa.starts_at::date between p_start_date and p_end_date and sa.ends_at<=now()
    and not private.sf_is_time_month_closed(sa.company_id,sa.starts_at::date)
    and (
  ...
  with candidates as (
    ...
    select company_id,
      ...
      jsonb_build_object('bulk',true,'comment',left(coalesce(p_note,''),1000))
    from inserted
    returning 1
  )
  ...
  return jsonb_build_object(
    ...
    'skippedClosed',v_skipped_closed,
    'skippedUnpublished',v_skipped_unpublished,
...
is 'Atomically copies planned times into missing manager time entries for completed published shifts.';
