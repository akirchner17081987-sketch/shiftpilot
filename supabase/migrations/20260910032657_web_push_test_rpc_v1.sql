...
begin
  ...
  if not exists(select 1 from public.push_subscriptions ps where ps.company_id=p_company_id and ps.user_id=v_user and ps.enabled=true) then raise exception 'PUSH_NOT_ACTIVE'; end if;
  if exists(
