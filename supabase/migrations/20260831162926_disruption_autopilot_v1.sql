...
create unique index if not exists disruption_one_open_incident_per_assignment on public.disruption_incidents (assignment_id) where status = 'OPEN';
create index if not exists disruption_incidents_company_status_idx on public.disruption_incidents (company_id, status, created_at desc);
create index if not exists disruption_incidents_original_employee_idx on public.disruption_incidents (original_employee_id);
create index if not exists disruption_incidents_created_by_idx on public.disruption_incidents (created_by);
create index if not exists disruption_incidents_resolved_by_idx on public.disruption_incidents (resolved_by) where resolved_by is not null;
...
create index if not exists disruption_offers_company_status_idx on public.disruption_offers (company_id, status, offered_at desc);
create index if not exists disruption_offers_employee_status_idx on public.disruption_offers (employee_id, status, offered_at desc);
create index if not exists disruption_offers_incident_idx on public.disruption_offers (incident_id, status);
create index if not exists disruption_offers_offered_by_idx on public.disruption_offers (offered_by);
...
create policy disruption_incidents_manager_select on public.disruption_incidents for select to authenticated using (
  company_id in (select cm.company_id from public.company_members cm where cm.user_id = (select auth.uid()) and cm.status = 'ACTIVE' and cm.role in ('OWNER','ADMIN','DISPATCHER','PLANNER'))
);
drop policy if exists disruption_offers_authorized_select on public.disruption_offers;
create policy disruption_offers_authorized_select on public.disruption_offers for select to authenticated using (
  employee_id in (select e.id from public.employees e where e.auth_user_id = (select auth.uid()) and e.status = 'active')
  or company_id in (select cm.company_id from public.company_members cm where cm.user_id = (select auth.uid()) and cm.status = 'ACTIVE' and cm.role in ('OWNER','ADMIN','DISPATCHER','PLANNER'))
);

create or replace function private.manager_create_disruption_impl(p_company_id uuid,p_assignment_id uuid,p_incident_type text,p_note text default '') returns uuid language plpgsql security definer set search_path = '' as $$
declare v_uid uuid:=auth.uid(); v_role text; v_assignment public.shift_assignments%rowtype; v_id uuid; v_type text:=upper(trim(coalesce(p_incident_type,'')));
begin
  select cm.role into v_role from public.company_members cm where cm.company_id=p_company_id and cm.user_id=v_uid and cm.status='ACTIVE' and cm.role in ('OWNER','ADMIN','DISPATCHER','PLANNER') limit 1;
  ...
  select * into v_assignment from public.shift_assignments where id=p_assignment_id and company_id=p_company_id for update;
  ...
  values(p_company_id,v_assignment.id,v_assignment.employee_id,v_assignment.version,v_type,left(coalesce(p_note,''),1000),'OPEN',v_uid) returning id into v_id;
  insert into public.audit_events(company_id,event_type,entity_type,entity_id,actor_id,actor_role,new_values)
  values(p_company_id,'DISRUPTION_CREATED','disruption_incident',v_id,v_uid,v_role,jsonb_build_object('assignmentId',v_assignment.id,'incidentType',v_type,'status','OPEN'));
  return v_id;
end $$;
...
begin
  ...
  where d.company_id=p_company_id group by d.id,a.id,o.id order by case when d.status='OPEN' then 0 else 1 end,d.created_at desc limit 100;
end $$;
...
begin
  ...
  return query with candidate as (
    select e.*,coalesce((select sum(extract(epoch from (s.ends_at-s.starts_at))/3600-s.break_minutes/60.0) from public.shift_assignments s where s.employee_id=e.id and s.status='PUBLISHED' and s.starts_at>=date_trunc('week',v_assignment.starts_at) and s.starts_at<date_trunc('week',v_assignment.starts_at)+interval '7 days'),0)::numeric(7,2) hours,
      case when lower(v_assignment.shift_code)='teamleiter' and e.role!~*'teamleiter|schichtleiter' then 'Teamleiterrolle erforderlich' else private.sf_swap_candidate_reason(v_assignment.id,e.id) end block_reason
    from public.employees e where e.company_id=v_incident.company_id and e.status='active' and e.id<>v_incident.original_employee_id and e.auth_user_id is not null
  )
  select c.id,trim(c.first_name||' '||c.last_name),c.role,(100-least(45,round(c.hours)::integer)+case when c.role~*'teamleiter|schichtleiter' then 5 else 0 end)::integer,c.hours,
    array['Schichtfreigabe vorhanden','Ruhezeit und Verfügbarkeit geprüft',trim(to_char(c.hours,'FM999990D0'))||' h in der Schichtwoche geplant']::text[],x.status
  from candidate c left join public.disruption_offers x on x.incident_id=v_incident.id and x.employee_id=c.id
  where c.block_reason is null order by 4 desc,c.last_name,c.first_name limit 12;
end $$;

create or replace function private.manager_send_disruption_offers_impl(p_incident_id uuid,p_employee_ids uuid[]) returns integer language plpgsql security definer set search_path = '' as $$
declare v_uid uuid:=auth.uid(); v_role text; v_incident public.disruption_incidents%rowtype; v_assignment public.shift_assignments%rowtype; v_employee public.employees%rowtype; v_employee_id uuid; v_reason text; v_count integer:=0; v_offer_id uuid; v_hours numeric; v_score integer;
begin
  ...
  for v_employee_id in select distinct unnest(p_employee_ids) loop
    ...
    v_reason:=private.sf_swap_candidate_reason(v_assignment.id,v_employee.id); if v_reason is not null then raise exception '%: %',trim(v_employee.first_name||' '||v_employee.last_name),v_reason; end if;
    ...
    on conflict(incident_id,employee_id) do update set rank_score=excluded.rank_score,rank_reasons=excluded.rank_reasons,status='OFFERED',offered_by=v_uid,offered_at=now(),expires_at=excluded.expires_at,responded_at=null,employee_comment='',updated_at=now() returning id into v_offer_id;
    ...
    values(v_incident.company_id,v_employee.auth_user_id,v_employee.id,'DISRUPTION_OFFER','Dringende Ersatzanfrage',v_assignment.shift_code||' · '||to_char(v_assignment.starts_at at time zone 'Europe/Berlin','DD.MM.YYYY HH24:MI'),'employee-disruptions','disruption_offer',v_offer_id,jsonb_build_object('incidentId',v_incident.id,'assignmentId',v_assignment.id)) on conflict do nothing;
    v_count:=v_count+1;
  end loop;
  insert into public.audit_events(company_id,event_type,entity_type,entity_id,actor_id,actor_role,metadata) values(v_incident.company_id,'DISRUPTION_OFFERS_SENT','disruption_incident',v_incident.id,v_uid,v_role,jsonb_build_object('count',v_count,'employeeIds',p_employee_ids));
  return v_count;
end $$;

create or replace function private.manager_cancel_disruption_impl(p_incident_id uuid,p_comment text default '') returns text language plpgsql security definer set search_path = '' as $$
...
create or replace function private.employee_list_disruption_offers_impl() returns table(offer_id uuid,offer_status text,incident_id uuid,shift_code text,starts_at timestamptz,ends_at timestamptz,incident_type text,note text,expires_at timestamptz,employee_comment text) language plpgsql security definer set search_path = '' as $$
...
create or replace function private.employee_respond_disruption_offer_impl(p_offer_id uuid,p_decision text,p_comment text default '') returns table(status text,message text,assignment_id uuid) language plpgsql security definer set search_path = '' as $$
...
begin
  ...
  update public.disruption_offers set status='EXPIRED',updated_at=now() where incident_id=v_incident.id and id<>v_offer.id and status='OFFERED';
