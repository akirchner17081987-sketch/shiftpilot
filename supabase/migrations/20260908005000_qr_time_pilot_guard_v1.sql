-- SchichtFunk – kontrollierter QR-Pilotbetrieb V1
-- Aktiviert QR-Zeiterfassung ausschließlich für ein ausdrücklich freigegebenes
-- Terminal + genau einen aktiven, mit Auth verknüpften Pilot-Mitarbeiter.
...
alter table public.time_qr_terminals
  ...
  check (not is_active or pilot_employee_id is not null);

create index if not exists time_qr_terminals_pilot_employee_idx
  on public.time_qr_terminals(company_id, pilot_employee_id)
  where pilot_employee_id is not null;
...
declare
  ...
  v_pilot public.employees%rowtype;
begin
  ...
  if v_terminal.pilot_employee_id is null then
    raise exception 'Der QR-Pilot ist für dieses Terminal noch nicht freigegeben';
  end if;

  select e.* into v_pilot
  from public.employees e
  where e.id = v_terminal.pilot_employee_id
    and e.company_id = v_terminal.company_id
    and e.status = 'active'
    and e.auth_user_id is not null;

  if not found then
    raise exception 'Der freigegebene Pilot-Mitarbeiter besitzt keinen aktiven, verknüpften Zugang';
  end if;

  if v_pilot.id <> v_employee_id then
    raise exception 'Dieses QR-Terminal ist aktuell nur für den freigegebenen Pilot-Mitarbeiter nutzbar';
  end if;
end;
$$;
...
-- Statusabfrage ebenfalls pilotgeschützt, damit nicht freigegebene Mitarbeiter
-- keine Schichtdetails über einen fotografierten/geteilten QR-Code sehen können.
...
-- Bestehende Buchungslogik bleibt unverändert erhalten, wird aber hinter eine
-- serverseitige Pilot-Freigabe gelegt. Die interne Basisfunktion ist nicht direkt
-- für authentifizierte Clients ausführbar.
alter function public.employee_clock_from_qr(text,text)
  rename to employee_clock_from_qr_unchecked;
...
-- Defense in depth: Selbst wenn die interne Basisfunktion später versehentlich
-- wieder ausführbar würde, darf kein QR-Punch außerhalb der Pilotfreigabe entstehen.
...
begin
  ...
  if not found or v_terminal.pilot_mode is not true or v_terminal.pilot_employee_id is null then
    raise exception 'QR-Pilotfreigabe fehlt';
  end if;

  if v_terminal.pilot_employee_id <> new.employee_id then
  ...
  if not found then
    raise exception 'Pilot-Mitarbeiter besitzt keinen aktiven, verknüpften Zugang';
  end if;
...
create trigger time_qr_punches_pilot_guard
before insert on public.time_qr_punches
for each row execute function private.sf_enforce_qr_punch_pilot();

-- Neue Terminals sind grundsätzlich deaktiviert und im Pilotmodus.
...
begin
  ...
  insert into public.time_qr_terminals(
    company_id, name, location_note, token_hash, is_active, pilot_mode,
    pilot_employee_id, created_by, updated_by
  ) values(
    p_company_id, left(btrim(p_name),120), left(btrim(coalesce(p_location_note,'')),300),
    extensions.digest(v_token,'sha256'), false, true, null, auth.uid(), auth.uid()
  ...
  insert into public.audit_events(
    ...
    jsonb_build_object('name',v_terminal.name,'location_note',v_terminal.location_note,
      'is_active',false,'pilot_mode',true),'{}'::jsonb
  );

  return jsonb_build_object(
    'id',v_terminal.id,'name',v_terminal.name,'location_note',v_terminal.location_note,
    'is_active',false,'pilot_mode',true,'token',v_token,'qr_path','/qr-time.html?t='||v_token
  );
end;
$$;
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'pilot_employee_name',case when e.id is null then null else btrim(coalesce(e.first_name,'') || ' ' || coalesce(e.last_name,'')) end,
  ...
  left join public.employees e
    on e.id = t.pilot_employee_id and e.company_id = t.company_id
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    'id',e.id,
    'display_name',btrim(coalesce(e.first_name,'') || ' ' || coalesce(e.last_name,'')),
    'personnel_no',e.personnel_no
  ) order by e.last_name,e.first_name,e.personnel_no),'[]'::jsonb)
  into v_result
  from public.employees e
...
begin
  ...
  if p_employee_id is null then
    if v_terminal.is_active then
      raise exception 'Aktives Pilot-Terminal kann nicht ohne Pilot-Mitarbeiter gespeichert werden';
    end if;
  else
    select e.* into v_employee
    from public.employees e
  ...
  set pilot_mode = true,
      pilot_employee_id = p_employee_id,
  ...
  insert into public.audit_events(
    ...
    jsonb_build_object('name',v_terminal.name,'pilot_mode',true,'pilot_employee_id',v_terminal.pilot_employee_id),
    '{}'::jsonb
  );

  return jsonb_build_object(
    'id',v_terminal.id,'name',v_terminal.name,'pilot_mode',true,
    'pilot_employee_id',v_terminal.pilot_employee_id
  );
end;
$$;
...
begin
  ...
  if p_is_active then
    ...
    if v_terminal.pilot_employee_id is null then
      raise exception 'Bitte zuerst einen Pilot-Mitarbeiter freigeben';
    end if;
    ...
    if not found then
      raise exception 'Der Pilot-Mitarbeiter ist nicht mehr aktiv oder besitzt keinen verknüpften Zugang';
    end if;
  end if;
  ...
  insert into public.audit_events(
    ...
    jsonb_build_object('name',v_terminal.name,'is_active',v_terminal.is_active,
      'pilot_mode',v_terminal.pilot_mode,'pilot_employee_id',v_terminal.pilot_employee_id),'{}'::jsonb
  );

  return jsonb_build_object(
    'id',v_terminal.id,'name',v_terminal.name,'is_active',v_terminal.is_active,
    'pilot_mode',v_terminal.pilot_mode,'pilot_employee_id',v_terminal.pilot_employee_id
  );
end;
$$;
...
comment on column public.time_qr_terminals.pilot_mode is
  'True while QR time tracking is restricted to a specifically assigned pilot employee.';
comment on column public.time_qr_terminals.pilot_employee_id is
  'Single employee allowed to use this QR terminal during controlled pilot rollout.';
