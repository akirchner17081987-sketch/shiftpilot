...
create index if not exists time_qr_pilot_employees_company_idx on public.time_qr_pilot_employees(company_id,employee_id);
...
select t.id,t.pilot_employee_id,t.company_id,t.updated_by from public.time_qr_terminals t where t.pilot_employee_id is not null
...
declare v_terminal_company uuid; v_employee public.employees%rowtype;
begin
  select t.company_id into v_terminal_company from public.time_qr_terminals t where t.id=new.terminal_id;
  if v_terminal_company is null or v_terminal_company<>new.company_id then raise exception 'QR-Terminal gehört nicht zum angegebenen Unternehmen'; end if;
...
create trigger time_qr_pilot_employees_validate before insert or update on public.time_qr_pilot_employees for each row execute function private.sf_validate_qr_pilot_member();

create or replace function private.sf_assert_qr_pilot_access(p_token text)
returns void language plpgsql security definer set search_path='' as $$
declare v_employee_id uuid; v_terminal public.time_qr_terminals%rowtype;
begin
  ...
  if not exists (
    select 1 from public.time_qr_pilot_employees p
    join public.employees e on e.id=p.employee_id and e.company_id=p.company_id and e.status='active' and e.auth_user_id is not null
    where p.terminal_id=v_terminal.id and p.company_id=v_terminal.company_id and p.employee_id=v_employee_id
  ) then raise exception 'Dieses QR-Terminal ist aktuell nur für ausdrücklich freigegebene Pilot-Mitarbeiter nutzbar'; end if;
end;$$;
...
begin
  ...
  if not found or v_terminal.pilot_mode is not true then raise exception 'QR-Pilotfreigabe fehlt'; end if;
  if not exists (
    select 1 from public.time_qr_pilot_employees p
    join public.employees e on e.id=p.employee_id and e.company_id=p.company_id and e.status='active' and e.auth_user_id is not null
    where p.terminal_id=new.terminal_id and p.company_id=new.company_id and p.employee_id=new.employee_id
  ) then raise exception 'Mitarbeiter ist für dieses QR-Terminal nicht freigegeben'; end if;
  return new;
end;$$;
...
create or replace function public.manager_set_time_qr_terminal_pilot_employees(p_terminal_id uuid,p_employee_ids uuid[])
returns jsonb language plpgsql security definer set search_path='' as $$
declare v_terminal public.time_qr_terminals%rowtype; v_ids uuid[]; v_current_ids uuid[]; v_valid_count integer; v_expected_count integer;
begin
  ...
  select coalesce(array_agg(distinct x order by x),'{}'::uuid[]) into v_ids from unnest(coalesce(p_employee_ids,'{}'::uuid[])) as x;
  select coalesce(array_agg(p.employee_id order by p.employee_id),'{}'::uuid[]) into v_current_ids from public.time_qr_pilot_employees p where p.terminal_id=v_terminal.id and p.company_id=v_terminal.company_id;
  if v_terminal.is_active and v_ids is distinct from v_current_ids then raise exception 'Pilot-Mitarbeiter können bei aktivem Terminal nicht geändert werden. Terminal zuerst deaktivieren.'; end if;
  v_expected_count:=coalesce(array_length(v_ids,1),0);
  if v_expected_count>10 then raise exception 'Maximal 10 Pilot-Mitarbeiter je Terminal sind zulässig'; end if;
  if v_expected_count>0 then
    select count(*) into v_valid_count from public.employees e where e.id=any(v_ids) and e.company_id=v_terminal.company_id and e.status='active' and e.auth_user_id is not null;
    if v_valid_count<>v_expected_count then raise exception 'Alle Pilot-Mitarbeiter müssen aktiv sein, zum Unternehmen gehören und einen verknüpften SchichtFunk-Zugang besitzen'; end if;
  end if;
  delete from public.time_qr_pilot_employees where terminal_id=v_terminal.id and company_id=v_terminal.company_id;
  if v_expected_count>0 then
    insert into public.time_qr_pilot_employees(terminal_id,employee_id,company_id,created_by)
    select v_terminal.id,x,v_terminal.company_id,auth.uid() from unnest(v_ids) as x;
  end if;
  update public.time_qr_terminals set pilot_mode=true,pilot_employee_id=case when v_expected_count>0 then v_ids[1] else null end,updated_at=clock_timestamp(),updated_by=auth.uid() where id=v_terminal.id;
  insert into public.audit_events(company_id,event_type,entity_type,entity_id,actor_id,actor_role,new_values,metadata)
  values(v_terminal.company_id,'TIME_QR_TERMINAL_PILOT_CHANGED','time_qr_terminal',v_terminal.id,auth.uid(),'MANAGER',jsonb_build_object('name',v_terminal.name,'pilot_mode',true,'pilot_employee_ids',to_jsonb(v_ids)),'{}'::jsonb);
  return jsonb_build_object('id',v_terminal.id,'name',v_terminal.name,'pilot_mode',true,'pilot_employee_ids',to_jsonb(v_ids));
end;$$;
...
returns jsonb language plpgsql security definer set search_path='' as $$
begin
  return public.manager_set_time_qr_terminal_pilot_employees(p_terminal_id,case when p_employee_id is null then '{}'::uuid[] else array[p_employee_id]::uuid[] end);
end;$$;
...
returns jsonb language plpgsql security definer set search_path='' as $$
declare v_terminal public.time_qr_terminals%rowtype; v_pilot_count integer;
begin
  select t.* into v_terminal from public.time_qr_terminals t where t.id=p_terminal_id for update;
  if not found or not private.sf_is_manager(v_terminal.company_id,true) then raise exception 'Keine Berechtigung'; end if;
  if p_is_active then
    ...
    select count(*) into v_pilot_count from public.time_qr_pilot_employees p
    join public.employees e on e.id=p.employee_id and e.company_id=p.company_id and e.status='active' and e.auth_user_id is not null
    where p.terminal_id=v_terminal.id and p.company_id=v_terminal.company_id;
    if v_pilot_count<1 then raise exception 'Bitte zuerst mindestens einen Pilot-Mitarbeiter freigeben'; end if;
  end if;
  ...
  insert into public.audit_events(company_id,event_type,entity_type,entity_id,actor_id,actor_role,new_values,metadata)
  values(v_terminal.company_id,'TIME_QR_TERMINAL_STATUS_CHANGED','time_qr_terminal',v_terminal.id,auth.uid(),'MANAGER',jsonb_build_object('name',v_terminal.name,'is_active',v_terminal.is_active,'pilot_mode',v_terminal.pilot_mode),'{}'::jsonb);
  return jsonb_build_object('id',v_terminal.id,'name',v_terminal.name,'is_active',v_terminal.is_active,'pilot_mode',v_terminal.pilot_mode);
end;$$;
...
comment on table public.time_qr_pilot_employees is 'Serverseitige Whitelist der ausdrücklich freigegebenen Mitarbeiter je QR-Pilotterminal.';;
