...
- Für Löschung/Offboarding liegt eine noch nicht produktiv ausgerollte private V1–V5-Grundlage mit idempotenter Warteschlange, Kunden-Fristprofil, Legal Holds, Zwei-Personen-Freigabe und vollständiger Vorschau vor. Für den tatsächlichen Betrieb mit genau einem echten OWNER ist zusätzlich `SOLE_OWNER_DELAYED` verbindlich festgelegt: zwei AAL2-Bestätigungen aus unterschiedlichen Sitzungen, 24 Stunden Abkühlfrist, 7 Tage Bestätigungsfenster, unveränderte Vorschau und harte Sperren gegen die Löschung des einzigen OWNER oder des Mandanten. Dies ist keine Vier-Augen-Kontrolle. V6 setzt die Regel einschließlich der ebenfalls zeitversetzten Fristprofilfreigabe im Prüfzweig technisch um; 22 fiktive, vollständig zurückrollende Datenbankfälle sind vorbereitet, aber mangels aktiver Testdatenbank noch nicht ausgeführt. `ACCESS` sperrt den Zugang nach gültiger Bestätigung; `ERASURE` wird getrennt nach Frist und Legal-Hold-Prüfung ausgeführt. Auth-Referenzen werden nach `RESTRICT/NO ACTION`, `CASCADE` und `SET NULL` getrennt; fremde Mandantenzugänge bleiben erhalten. Der frühere Wegwerf-Testbranch bestand 41/41 Lifecycle-/Offboarding-/MFA-Prüfungen. V6, Auth-Admin-/Personalakten-Storage-Schritt, Langfristredaktion und Zeitplanung sind bewusst noch nicht produktiv aktiv.
...
2. Lösch- und Aufbewahrungskonzept technisch fertigstellen: Die V1–V5-Grundlage und erste Datenbankausführung sind auf dem früheren Wegwerf-Testbranch geprüft; die Ein-OWNER-Betriebsregel und V6 sind im Prüfzweig vorbereitet. Als Nächstes die 22 V6-Fälle auf einem kostenpflichtigen Wegwerf-Testbranch ausführen. Danach bleiben Audit-/Monatssnapshot-Redaktion, Auth-Admin-/Personalakten-Storage-Koordination, Zeitplanung und kundenspezifische Langfristfristen offen; keine Testbranch-Buchung oder Produktivaktivierung ohne gesonderte Freigabe.
