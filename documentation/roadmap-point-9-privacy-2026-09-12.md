...
- Für Löschung/Offboarding liegt eine noch nicht ausgerollte private V1–V3-Grundlage mit idempotenter Warteschlange, versioniertem Kunden-Fristprofil, Legal Holds, Vier-Augen-Freigabe und rein lesendem Mitarbeiter-Dry-Run vor. V3 inventarisiert Planungs-, Zeit-, QR-, Personalakten-, Push-, Audit-, Storage- und Auth-Verknüpfungen. Auth-Referenzen werden nach `RESTRICT/NO ACTION`, `CASCADE` und `SET NULL` getrennt; Mandanteneigentum, weitere Mitgliedschaften/Profile oder nicht bereinigte blockierende/mitlöschende Referenzen verhindern die Freigabe einer Kontenlöschung. Der vorbereitete Edge-Endpunkt prüft aktive OWNER/ADMIN und verlangt `aal2` für Auftrag und Freigabe. Ein fiktiver Mandantentest deckt Fremdmandant, falsche Rolle und fehlendes MFA ab. Fällige Aufträge können mit `SKIP LOCKED` sicher von einem Hintergrundarbeiter übernommen werden; eine Löschroutine und Zeitplanung sind bewusst noch nicht aktiv.
...
- TOTP-MFA und die 15-Minuten-Begrenzung reiner AAL1-Sitzungen sind in Supabase aktiv. Die IONOS-Vorschau enthält seit Commit `ff20020` Einrichtung, Faktorenverwaltung und den Login-Challenge-Schritt für App-Authenticator; Build, Deployment sowie öffentliche Auslieferung der neuen Datei wurden bestätigt. Die Datenschutzfreigabe verlangt bereits serverseitig `aal2`; der echte Konten-/Gerätetest und die Ausweitung auf weitere sensible Bereiche stehen aus. Leaked-Password-Schutz und sichere Passwortänderung sind weiterhin aus.
...
3. Lösch- und Aufbewahrungskonzept technisch fertigstellen: V1–V3-Grundlage auf einer Wegwerf-Testumgebung ausführen; Audit-/Monatssnapshot-Redaktion und die koordinierte Auth-/Storage-/Push-/Datenbankausführung ergänzen. Fristprofile, Vier-Augen-Freigabe, Legal Holds und die referenzsichere Auth-Vorprüfung sind vorbereitet, aber nicht produktiv ausgerollt.
