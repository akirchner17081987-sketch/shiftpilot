...
Supabase-seitig ist TOTP bereits verfügbar. In der IONOS-Vorschau für Git-Commit `ff20020` liegt eine App-Oberfläche für Einrichtung per QR/Secret, Bestätigung des sechsstelligen Codes, Faktorenliste, Entfernung und den verpflichtenden Challenge-Schritt bei bereits registriertem Faktor vor. Build und IONOS-Deployment waren erfolgreich; Datei und Loader-Einbindung wurden öffentlich mit HTTP 200 geprüft. Abgebrochene Anmeldungen beenden die lokale Sitzung; unvollständig eingerichtete TOTP-Faktoren werden beim nächsten Einrichtungsversuch bereinigt. Der Hosted-Auth-Fluss wurde mit zwei synthetischen Konten einschließlich Ersatzfaktor und Sitzungswiderruf live geprüft; die App-Integration ist zusätzlich statisch abgesichert. Beim einzigen echten `OWNER` wurde am 14.09.2026 ein verifizierter TOTP-Faktor bestätigt. Stufe 2 der serverseitigen Pflicht ist produktiv aktiv; die organisatorische Freigabe des Wiederherstellungswegs und die Produktivaktivierung der späteren Stufen fehlen weiterhin.
...
Die Datenschutz-Edge-Function verlangt für Auftrag und Freigabe bereits eine verifizierte `aal2`-Sitzung. Für 27 weitere sensible Data-API-RPCs ist eine zentrale, stufenweise Pre-Request-Grenze vorbereitet. Stufe 2 mit vier RPCs der Benutzer- und Rechteverwaltung wurde am 14.09.2026 produktiv aktiviert; die Stufen 3 bis 5 bleiben deaktiviert. Deshalb darf MFA für die späteren Funktionsgruppen noch nicht als vollständig umgesetzt gelten.
...
Eine präzisierte rein lesende Produktivprüfung am 14.09.2026 ergab genau ein echtes `OWNER`-Konto für den Mandanten `SchichtFunk`, kein aktives `ADMIN`-Konto und – nach der persönlichen Einrichtung – einen verifizierten TOTP-Faktor für diesen echten OWNER. Die Verifizierung erfolgte um 05:23:54 UTC. Der zweite zuvor mitgezählte OWNER gehört zu einem getrennten Mandanten mit der ausdrücklichen Kennzeichnung `SchichtFunk Abnahme … (FIKTIV)` und ist kein zweiter produktiver Inhaber. Die verpflichtende App-Oberfläche ist vorbereitet. Nach ausdrücklicher Freigabe wurde die Serverpflicht für Stufe 2 produktiv aktiviert; Stufen 3 bis 5 bleiben aus. Die vier RPC-Stufen bestanden zuvor auf einem datenlosen Wegwerf-Branch 59/59 Assertions einschließlich echtem PostgREST-Data-API-Negativtest. Die Produktivkontrolle für Stufe 2 bestätigte zusätzlich AAL1-Ablehnung, AAL2-Zulassung, Service-Role-Zulassung und den unveränderten Stufe-3-Pfad ohne Geschäftsdatenänderung.
...
Status: 🟢 **LEAKED PASSWORD PROTECTION, ECHTKONTO-MFA, ZWEI-KONTEN-MFA-/RECOVERY-/SITZUNGSWIDERRUF-NACHWEIS, 59/59 AAL2-BRANCH-ASSERTIONS UND PRODUKTIVE STUFE 2 BESTANDEN**; 🟡 **SICHERE PASSWORTÄNDERUNG UND AAL2-STUFEN 3 BIS 5 NOCH OFFEN.**
